package com.barribob.intothemaelstrom.entity.model;
...
public class ModelAnimatedBiped<T extends ModelBase>  extends ModelBiped {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        ...
        if (entityIn instanceof EntityLeveledMob) {
            ((EntityLeveledMob) entityIn).getCurrentAnimation().setModelRotations(this, limbSwing, limbSwingAmount, ((EntityLeveledMob<T>) entityIn).getHealth() > 0 ? this.partialTicks : 0.99f);
        } else {
