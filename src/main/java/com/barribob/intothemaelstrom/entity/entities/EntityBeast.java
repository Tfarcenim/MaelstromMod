...
import com.barribob.intothemaelstrom.entity.model.ModelBeast;
import com.barribob.intothemaelstrom.entity.projectile.ProjectileEntityBeastAttack;
...
public class EntityBeast extends EntityMaelstromMob {
    ...
    public EntityBeast(World worldIn) {
        ...
        if (!worldIn.isRemote) {
            ...
            attackHandler.setAttack(spit, (IAction) (actor, target) -> {
                for (int i = 0; i < 5; i++) {
                    ProjectileEntityBeastAttack projectile = new ProjectileEntityBeastAttack(actor.world, actor, actor.getAttack() * getConfigDouble("spit_damage"));
    ...
    protected void initAnimation() {
        List<List<AnimationClip<ModelBeast>>> animationLeap = new ArrayList<>();
        List<AnimationClip<ModelBeast>> head = new ArrayList<>();
        ...
        List<List<AnimationClip<ModelBeast>>> animationSpit = new ArrayList<>();
        List<AnimationClip<ModelBeast>> jaw = new ArrayList<>();
    ...
    public void onLivingUpdate() {
        if (!world.isRemote && this.isLeaping()) {
            ModUtils.handleAreaImpact(2.5f, (e) -> this.getAttack() * getConfigDouble("leap_damage"), this,
