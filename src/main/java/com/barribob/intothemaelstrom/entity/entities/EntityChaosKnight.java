...
import com.barribob.intothemaelstrom.entity.projectile.ProjectileEntityChaosFireball;
...
import com.barribob.intothemaelstrom.world.gen.nexus.WorldGenNexusTeleporter;
...
public class EntityChaosKnight extends EntityMaelstromMob implements IAttack, DirectionalRender, ITarget {
    ...
    private final Consumer<EntityLivingBase> leapSlam = (target) -> {
        ...
        addEvent(() -> {
            ...
            double damage = this.getAttack() * getConfigDouble("leap_slam_damage");
    ...
    private final Consumer<EntityLivingBase> sideSwipe = (target) -> {
        ...
        addEvent(() -> {
            ...
            double damage = getAttack() * getConfigDouble("single_swipe_damage");
        ...
        addEvent(() -> {
            ...
            ModUtils.lineCallback(startPos, chargeDir, (int) Math.sqrt(chargeDir.subtract(startPos).lengthSquared()), (vec, i) -> {
                DamageSource source = ModDamageSource.builder()
                        ...
                        .element(getElement())
                        .stoppedByArmorNotShields().build();
                double damage = getAttack() * getConfigDouble("dash_damage");
    ...
    private final Consumer<EntityLivingBase> spinSlash = (target) -> {
        ...
        Runnable meleeAttack = () -> {
            ...
            double damage = getAttack() * getConfigDouble("spin_slash_damage");
    ...
    private final Consumer<EntityLivingBase> summonMeteors = (target) -> {
        ...
        for (int tick = 20; tick < 140; tick += 5) {
            addEvent(() -> {
                double damage = this.getAttack() * getConfigDouble("meteor_damage");
                ProjectileEntityChaosFireball meteor = new ProjectileEntityChaosFireball(world, this, damage, null);
    ...
    private static final SoundEvent[] ambient = new SoundEvent[]{ModSoundEvents.ENTITY_CHAOS_KNIGHT_AMBIENT1,ModSoundEvents.ENTITY_CHAOS_KNIGHT_AMBIENT2,ModSoundEvents.ENTITY_CHAOS_KNIGHT_AMBIENT3};
    private static final SoundEvent[] hurt = new SoundEvent[]{ModSoundEvents.ENTITY_CHAOS_KNIGHT_HURT1,ModSoundEvents.ENTITY_CHAOS_KNIGHT_HURT2,ModSoundEvents.ENTITY_CHAOS_KNIGHT_HURT3};
    private static final SoundEvent[] death = new SoundEvent[]{ModSoundEvents.ENTITY_CHAOS_KNIGHT_DEATH1,ModSoundEvents.ENTITY_CHAOS_KNIGHT_DEATH2,ModSoundEvents.ENTITY_CHAOS_KNIGHT_DEATH3};
