...
import com.barribob.intothemaelstrom.entity.animation.AnimationFireballThrow;
import com.barribob.intothemaelstrom.entity.animation.AnimationHerobrineGroundSlash;
import com.barribob.intothemaelstrom.entity.animation.AnimationSpinSlash;
import com.barribob.intothemaelstrom.entity.projectile.ProjectileEntityFireball;
import com.barribob.intothemaelstrom.entity.projectile.ProjectileEntityHerobrineQuake;
...
public class EntityHerobrineOne extends EntityLeveledMob implements IRangedAttackMob {
    ...
    public EntityHerobrineOne(World worldIn) {
        ...
        if (!world.isRemote) {
            ...
            attackHandler.setAttack(fireball, (IAction) (actor, target) -> {
                actor.playSound(SoundEvents.ENTITY_BLAZE_SHOOT, 1.0F, 0.4F / (actor.world.rand.nextFloat() * 0.4F + 0.8F));

                ProjectileEntityFireball projectile = new ProjectileEntityFireball(actor.world, actor, actor.getAttack(), null);
    ...
    public void handleStatusUpdate(byte id) {
        if (id >= 4 && id <= 6) {
            ...
            for (int i = 0; i < fireballParticles; i++) {
                ...
                ParticleManager.spawnCustomSmoke(world, pos, ProjectileEntityFireball.FIREBALL_COLOR, Vec3d.ZERO);
            }
