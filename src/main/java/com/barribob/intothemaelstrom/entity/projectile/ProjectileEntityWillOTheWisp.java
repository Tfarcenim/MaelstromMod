 ...
 */
public class ProjectileEntityWillOTheWisp extends ProjectileEntityGun {
    private static final int PARTICLE_AMOUNT = 6;
    private static final int AREA_FACTOR = 2;

    public ProjectileEntityWillOTheWisp(World worldIn, EntityLivingBase throwerIn, float baseDamage, ItemStack stack) {
        super(worldIn, throwerIn, baseDamage, stack);
        this.setNoGravity(true);
    }

    public ProjectileEntityWillOTheWisp(World worldIn) {
        super(worldIn);
    }

    public ProjectileEntityWillOTheWisp(World worldIn, double x, double y, double z) {
        super(worldIn, x, y, z);
    }
        ...
        for (Object entity : list) {
            if (entity instanceof EntityLivingBase && this.shootingEntity != null && entity != this.shootingEntity) {
                ...
                ((EntityLivingBase) entity).attackEntityFrom(ModDamageSource.causeElementalThrownDamage(this, shootingEntity, getElement()),
                        (float) this.getGunDamage(((EntityLivingBase) entity)));
